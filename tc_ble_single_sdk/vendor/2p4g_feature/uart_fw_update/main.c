...
#if(FW_UPDATE_MASTER_BIN_ADDR == FW_UPDATE_BIN_ADDR_20000 || FW_UPDATE_SLAVE_BIN_ADDR == FW_UPDATE_BIN_ADDR_20000 )
    blc_ota_setFirmwareSizeAndBootAddress(124,FW_UPDATE_BIN_ADDR_20000);
#else
    blc_ota_setFirmwareSizeAndBootAddress(252,FW_UPDATE_BIN_ADDR_40000);
#endif
...
void app_flash_protection_operation(u8 flash_op_evt, u32 op_addr_begin, u32 op_addr_end)
{
    ...
    else if(flash_op_evt == FLASH_OP_EVT_STACK_OTA_CLEAR_OLD_FW_BEGIN)
    {
         ...
         * must unlock flash */
        tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] UART clear old FW begin, unlock flash\n");
        flash_unlock();
    }
    else if(flash_op_evt == FLASH_OP_EVT_STACK_OTA_CLEAR_OLD_FW_END)
    {
        ...
        tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] UART clear old FW end, restore flash locking\n");
        flash_lock(flash_lockBlock_cmd);
    }
    else if(flash_op_evt == FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_BEGIN)
    {
         ...
         * In this sample code, we protect whole flash area for old and new firmware, so here we do not need judge "op addr_begin" and "op addr_end",
         * must unlock flash */
        tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] UART write new FW begin, unlock flash\n");
        flash_unlock();
    }
    else if(flash_op_evt == FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END)
    {
        ...
        tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] UART write new FW end, restore flash locking\n");
        flash_lock(flash_lockBlock_cmd);
    }
#endif
