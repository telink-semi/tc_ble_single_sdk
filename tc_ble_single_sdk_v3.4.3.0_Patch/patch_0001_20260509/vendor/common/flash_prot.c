...
void flash_protection_init(void)
{
	...
	{
		#if (FLASH_ZB25WD10B_SUPPORT_EN)
			case MID11325E:
				flash_lock_mid = (flash_lock_t)flash_lock_mid11325e;
				flash_unlock_mid = (flash_unlock_t)flash_unlock_mid11325e;
				flash_get_lock_status_mid = (flash_get_lock_status_t)flash_get_lock_block_mid11325e;
				flash_unlock_status = FLASH_LOCK_NONE_MID11325E;
				break;
		#endif
		...
		#if (FLASH_GD25LD05C_SUPPORT_EN)
			case MID1060C8:
				flash_lock_mid = (flash_lock_t)flash_lock_mid1060c8;
				flash_unlock_mid = (flash_unlock_t)flash_unlock_mid1060c8;
				flash_get_lock_status_mid = (flash_get_lock_status_t)flash_get_lock_block_mid1060c8;
				flash_unlock_status = FLASH_LOCK_NONE_MID1060C8;
				break;
        #endif
		
		#if (FLASH_GD25LD10C_SUPPORT_EN || FLASH_GD25LD10E_SUPPORT_EN)
			case MID1160C8:
				flash_lock_mid = (flash_lock_t)flash_lock_mid1160c8;
				flash_unlock_mid = (flash_unlock_t)flash_unlock_mid1160c8;
...
u16 flash_change_app_lock_block_to_flash_lock_block(flash_app_lock_e app_lock_block)
{
	...
	switch(blc_flash_mid)
	{
		#if (FLASH_GD25LD05C_SUPPORT_EN) //64K capacity, Only for 2.4G
			case MID1060C8:
				if(app_lock_block == FLASH_LOCK_FW_LOW_32K){
					flash_lock_block_size = FLASH_LOCK_LOW_32K_MID1060C8;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock low 32K block!\n");
				}
				else if(app_lock_block == FLASH_LOCK_FW_LOW_64K){
					flash_lock_block_size = FLASH_LOCK_ALL_64K_MID1060C8;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock ALL 64K block!\n");
				}
				else{
					blc_flashProt.init_err = 1; 
				}
		...
		#if (FLASH_GD25LD10C_SUPPORT_EN || FLASH_GD25LD10E_SUPPORT_EN) //128K capacity, Only for 2.4G
			case MID1160C8:
				if(app_lock_block == FLASH_LOCK_FW_LOW_64K){
					flash_lock_block_size = FLASH_LOCK_LOW_64K_MID1160C8;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock low 64K block!\n");
				}
				else if(app_lock_block == FLASH_LOCK_FW_LOW_128K){
					flash_lock_block_size = FLASH_LOCK_ALL_128K_MID1160C8;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock ALL 128K block!\n");
				}
				else{
					blc_flashProt.init_err = 1; 
				}
		...
		#if (FLASH_ZB25WD10C_SUPPORT_EN)
			case MID11325E:
				if(app_lock_block == FLASH_LOCK_FW_LOW_64K){
					flash_lock_block_size = FLASH_LOCK_LOW_64K_MID11325E;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock low 64K block!\n");
				}
				else if(app_lock_block == FLASH_LOCK_FW_LOW_128K){
					flash_lock_block_size = FLASH_LOCK_ALL_128K_MID11325E;
					tlkapi_printf(APP_FLASH_PROT_LOG_EN, "[FLASH][PROT] flash lock ALL 128K block!\n");
				}
				else{
					blc_flashProt.init_err = 1; 
