...
// max value is 65535(16bit)
// Actual testing found that, in STX2RX mode, the maximum RX timeout is approximately 4 ms.
#define MAC_RX_WAIT                   4000 //in us
              ...
              const MAC_Cb RxFirstTimeoutCb)
{
    ...
    gen_fsk_radio_power_set(RF_POWER_N0p14dBm);
...
void MAC_RxIrqHandler(void)
{
    ...
    if (!gen_fsk_is_rx_crc_ok_register()) {
        if (mac_Info.RxCb) {
            mac_Info.RxCb(NULL);
        }
        return;
